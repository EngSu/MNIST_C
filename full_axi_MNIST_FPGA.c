...
#define GPIO_BASE_ADDRESS_0 	0x43C00000
#define GPIO_BASE_ADDRESS_1 	0x80000000
...
{   
        ...
        FILE *file,*outfile, *fp_output, *errfile0, *errfile1, *labelfile, *accfile0, *accfile1, *hw_labels, *sw_labels, *actual_labels, *timefile;
        double outvariable;
	int labelvariable;
        int myvar;
	...
	off_t dev_base0 = GPIO_BASE_ADDRESS_0;
        void *mapped_base1, *mapped_dev_base1; 
	off_t dev_base1 = GPIO_BASE_ADDRESS_1;  
...
//printf("%d:	%d \n",j, x_in[j]);
	...
	//*((unsigned long int *) (mapped_dev_base1 + GPIO_DATA_OFFSET + i + 16)) = (x_temp[jj][i+3]<<24)|(x_temp[jj][i+2]<<16)|(x_temp[jj][i+1]<<8)|(x_temp[jj][i]);
	gettimeofday(&start, NULL);
        memcpy((mapped_dev_base1 + GPIO_DATA_OFFSET),x_in,4*196);
        //armmemcpy((mapped_dev_base1 + GPIO_DATA_OFFSET),x_in,4*196);
