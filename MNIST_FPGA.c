...
{   
        ...
        FILE *file,*outfile, *fp_output, *errfile0, *errfile1, *labelfile, *accfile0, *accfile1, *hw_labels, *sw_labels, *actual_labels;
        double outvariable;
	int labelvariable;
        int myvar;
...
// send in_valid signal 0
	...
	for(ii = 0; ii < 196; ii=ii+1){  
		*((unsigned long int *) (mapped_dev_base + GPIO_DATA_OFFSET + i + 16)) = (x_temp[jj][i+3]<<24)|(x_temp[jj][i+2]<<16)|(x_temp[jj][i+1]<<8)|(x_temp[jj][i]);
                //*((volatile  signed int *) (mapped_dev_base + GPIO_DATA_OFFSET + i + 16)) = x_temp[jj][i];
	        //*((volatile  signed int *) (mapped_dev_base + GPIO_DATA_OFFSET + i + 17)) = x_temp[jj][i+1];
		//*((volatile  signed int *) (mapped_dev_base + GPIO_DATA_OFFSET + i + 18)) = x_temp[jj][i+2];
		//*((volatile  signed int *) (mapped_dev_base + GPIO_DATA_OFFSET + i + 19)) = x_temp[jj][i+3];
                //printf("Input %d:    %04x	 \n",i, (x_temp[jj][i]));
i =i+4;
...
// send in_valid signal 1
  ...
  // printf("output ready  :%d \n\r", *((unsigned long *) (mapped_dev_base + GPIO_DATA_OFFSET+ 12)));
             	 ...
             	 out_temp[0] =  *((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET +4)) ;
            	 out_temp[1] =  *((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET +8)) ;
		...
		//printf("Y_node%d = %f \n\n",j, sum_exp_val);
       		//}
	  	acc_out0 = round_f((float)(*((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET+ 4)))/128);
	  	acc_out1 = round_f((float)(*((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET+ 8)))/128);
	...
	//acc_out1 = (float)(*((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET+ 8)))>>7;
      //printf("Hardware Output %d: %.6f		%.6f \n",jj, acc_out0, acc_out1);
       fprintf(fp_output,"%d	%d\n", *((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET+ 4)),*((volatile signed int *) (mapped_dev_base + GPIO_DATA_OFFSET+ 8)));
  ...
  printf("=========================================================================\
\n Time Taken(sec) for accelerated-based MNIST with AXI_Lite: %f\n=========================================================================\n\
\n",secs);
